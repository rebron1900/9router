...
## 🍴 Об этом персональном форке

> [!IMPORTANT]
> Этот репозиторий — персональный форк, который поддерживает [rebron1900](https://github.com/rebron1900) на основе исходного проекта [decolua/9router](https://github.com/decolua/9router). Он содержит пользовательские функции и исправления совместимости, которых может не быть в upstream. Для этой версии используйте [Issues](https://github.com/rebron1900/9router/issues) и [Releases](https://github.com/rebron1900/9router/releases) данного репозитория.

### Основные обновления после создания форка

- **Единая маршрутизация моделей** — Управление едиными именами моделей, порядком привязки провайдеров, сопоставлением отдельных моделей, маршрутизацией по возможностям и видимостью провайдеров/моделей из панели управления.
- **Единое рабочее пространство изображений** — Генерация и редактирование изображений в одном интерфейсе, OpenAI-совместимые endpoints и поиск моделей с фильтрацией по возможностям.
- **Более надёжные маршрутизация и streaming** — Общие бюджеты повторов и времени, fallback аккаунтов и провайдеров, корректная обработка отключения клиента, keepalive для долгих forced-stream запросов и повтор CommandCode на том же аккаунте при временных ошибках gateway.
- **Более широкая совместимость протоколов и моделей** — Улучшено преобразование OpenAI Responses, Chat Completions и Claude, добавлена мультимодальность CommandCode и согласованы возможности runtime-маршрутизации с `/v1/models`.
- **Использование и диагностика** — Расширенная аналитика использования и сведения о запросах, а также опциональные отладочные логи.
- **Docker-релизы форка** — Мультиархитектурные образы публикуются в [GHCR](https://github.com/rebron1900/9router/pkgs/container/9router); каждый релиз обновляет тег версии и `latest`.
